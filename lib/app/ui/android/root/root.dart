...
class Root extends GetView<RootController> {
  ...
  Widget build(BuildContext context) {
    ...
    if (get_value!=null){
      if (get_value['before_page'] == 'logout') {
        controller.user_id = '';
      } else if (get_value['before_page'] == 'login') {
      ...
      print('user_id_bool :');
      print(user_id_bool);
      if (get_value['before_page'] == null || get_value['before_page'] == '' || get_value['before_page'] == 'logout') {
        value_bool = false;
        controller.changeRootPageIndex1(1);
      } else if (get_value['before_page'] == 'login') {
        value_bool = true;
        controller.changeRootPageIndex1(1);
      } else if (get_value['before_page'] == 'service') {
        //로그인하지 않고 서비스에서 넘어온다면
        controller.changeRootPageIndex1(2);
      } else if (get_value['before_page'] == 'book_item') {
        controller.changeRootPageIndex1(1);
      }
    }
      ...
      child: Obx(
        () => Scaffold(
          body: IndexedStack(
            ...
            children: [
              Search(),
              get_value!=null?controller.buildNavigator(get_value):controller.buildNavigator({'before_page':'null'}),
              (value_bool == false && user_id_bool == false)
                  ? Login()
                  : Mypage(),
            ],
          ),
          bottomNavigationBar: BottomNavigationBar(
            ...
            items: [
              ...
              BottomNavigationBarItem(
                ...
                label: (value_bool == false && user_id_bool == false)
                    ? 'Login'
                    : 'Mypage',
